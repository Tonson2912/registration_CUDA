...
struct IndexOffset
{
	...
	{
		if (indexX != NULL) {
			delete[] indexX; indexX = NULL;
		}
		if (indexY != NULL) {
			delete[] indexY; indexY = NULL;
		}
	}
...
void getDftupsIn(cufftComplex *buf1ft, cufftComplex *buf2ft, cufftComplex *in, unsigned int height, unsigned int width)
{
	...
	//cufftComplex *check = new cufftComplex[height*width]();
	//cudaMemcpy(check, in, sizeof(cufftComplex)*height*width, cudaMemcpyDeviceToHost);
	//for (int i = 0; i < width; ++i)
	//{
	//	std::cout << i + 1 << "  :" <<std::fixed<< check[i+1080*width].x << "  :" << check[i+1080*width].y << std::endl;
	//}
...
void bilinear(float *d_ImageIn, float x, float y, unsigned int oHeight, unsigned int oWidth)
{
	...
	//float *check = new float[oHeight*oWidth]();
	//cudaMemcpy(check, d_ImageIn, sizeof(float)*oHeight*oWidth, cudaMemcpyDeviceToHost);
	//for (int i = 0; i < oWidth; ++i)
	//{
	//	std::cout << i + 1 << "  :" << check[i] << std::endl;
	//}
...
void registration(float *ImageIn, float *ImageOut, cufftComplex *d_fixedFFT, unsigned int oHeight, unsigned int oWidth, unsigned int nHeight, unsigned int nWidth, unsigned int runImageNum)
{
	...
	dftregistration(ind, d_fixedFFT, d_ImageInFFT, handle, oHeight, oWidth, 100, runImageNum);//互相关找偏移量
	...
	for (int i = 0; i < runImageNum; ++i)
	{
		...
		//float *check = new float[nHeight*nWidth*runImageNum]();
		//cudaMemcpy(check, d_ImageOut, sizeof(float)*nHeight*nWidth*runImageNum, cudaMemcpyDeviceToHost);

// registrationDll.cpp : 定义 DLL 应用程序的导出函数。
//
...
void FTpad(cv::Mat& imFTout, cv::Mat& imFT, unsigned int outsizeH, unsigned int outsizeW)
{
	...
	//int outTp_height = MIN(cenout_cen.height + Nin.height, Nout.height) - 1 - outTp_y;
	...
	int inTp_height = MIN(-cenout_cen.height + Nout.height, Nin.height) - 1 - inTp_y;
	int inTp_width = MIN(-cenout_cen.width + Nout.width, Nin.width) - 1 - inTp_x;
